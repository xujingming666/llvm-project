...
public:
  ...
  bool selectAddrRegOffset(SDValue Addr, SDValue &Base,
...
// Match reg/frameindex+offset.
bool DummyDAGToDAGISel::selectAddrRegOffset(SDValue Addr, SDValue &Base,
                                          SDValue &Offset, unsigned OffsetBits,
                                          unsigned ShiftAmount) const {
  if (CurDAG->isBaseWithConstantOffset(Addr)) {

    llvm::errs() << "isBaseWithConstantOffset in \n";
    ConstantSDNode *CN = dyn_cast<ConstantSDNode>(Addr.getOperand(1));
    // we need to check the alignment.
    EVT ValTy = Addr.getValueType();

    // If the first operand is a FI, get the TargetFI Node
    Base = Addr.getOperand(0);

    Offset = CurDAG->getTargetConstant(CN->getZExtValue(),
                                        SDLoc(Addr), ValTy);
    return true;
  }
...
bool DummyDAGToDAGISel::SelectAddrImm12(SDValue Addr, SDValue &Base,
                                      SDValue &Offset) {
  llvm::errs() << "SelectAddrImm12: " << Addr.getOpcode() <<"\n";
  ...
  // if (selectAddrRegOffset(Addr, Base, Offset, 12))
  //   return true;

  if (Addr.getValueType() == MVT::i32) {
    llvm::errs() << "Addr.getValueType() == MVT::i32 \n";
    Base = Addr;
    Offset = CurDAG->getTargetConstant(0, SDLoc(Addr), MVT::i32);
    return true;
  }

  // if (GlobalAddressSDNode *global_addr = dyn_cast<GlobalAddressSDNode>(Addr)) {
  //   SDLoc DL(Addr);
  //   Base = CurDAG->getTargetGlobalAddress(global_addr->getGlobal(), DL, Addr.getValueType());
  //   Offset = CurDAG->getTargetConstant(global_addr->getOffset(), SDLoc(Addr), MVT::i32);
  //   return true; 
  // }
